...
func main1() {
	...
	resp, err := svc.ListObjectsV2(&s3.ListObjectsV2Input{Bucket: aws.String("pubg-account-data")})
	if err != nil {
	...
	for _, item := range resp.Contents {
		...
		if !strings.Contains(*item.Key, "pubg-account-data-temp") {
			srcKey := "/" + "pubg-account-data" + "/" + *item.Key
			destKey := "/pubg-account-data-temp/" + *item.Key

			fmt.Printf("Item  copying from bucket %q to bucket %q\n", srcKey, destKey)

			_, err = svc.CopyObject(&s3.CopyObjectInput{Bucket: aws.String("pubg-account-data"),
				CopySource: aws.String(srcKey),
				Key:        aws.String(destKey)})
			if err != nil {
				fmt.Printf("Unable to copy item  from bucket %q to bucket %q, %v", srcKey, destKey, err)
				return
			}
